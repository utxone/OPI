...
app.get('/v1/runes/get_unspent_rune_outpoints_of_wallet', async (request, response) => {
  try {
    ...
    let current_block_height = await get_block_height_of_db()
    let query = ` select pkscript, wallet_addr, rotb.outpoint, rotb.rune_ids, rotb.balances, ARRAY_AGG(rite.rune_name) as rune_names, ARRAY_AGG(rite.spacers) as rune_spacers, ARRAY_AGG(rite.symbol) as symbols 
                  from runes_outpoint_to_balances rotb join runes_id_to_entry rite on rite.rune_id = any(rotb.rune_ids)
                  where ` + pkscript_selector + ` = $1 and spent = false group by pkscript, wallet_addr, rotb.outpoint, rotb.rune_ids, rotb.balances;`
    let params = [pkscript_selector_value]
